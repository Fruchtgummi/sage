...
#   from, https:// URL)
# * SECRET_GIT_BINDER_KEY (an RSA private key with push access to GIT_BINDER)
...
if [[ -z "$GIT_BINDER" ]]; then
    echo "No git repository configured for binder builds. Not pushing binder configuration."
    exit 0
fi

# Restore private key for ${SECRET_GIT_BINDER_KEY}.
mkdir -p ~/.ssh/
cat "$SECRET_GIT_BINDER_KEY" | sed 's/\\n/\n/g' > ~/.ssh/id_rsa
chmod 700 ~/.ssh
