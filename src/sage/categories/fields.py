...
class Fields(Category_singleton):
    ...
    class ParentMethods:
        ...
        def is_perfect(self):
            """
            Return whether this field is perfect.

            OUTPUT:

            Returns ``True`` if this field has characteristic zero or if it has
            positive characteristic `p` and every element has a `p`-th root.
            ...
            raise NotImplementedError("is_perfect() not implemented for this field.")
        ...
        def _xgcd_univariate_polynomial(self, f, g):
            ...
            Polynomials ``r``, ``s``, and ``t`` such that ``r = s*f + t*g``
            ...
            R = f.parent()
            if g.is_zero():
                return f, R.one_element(), R.zero_element()
