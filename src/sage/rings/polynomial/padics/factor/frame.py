...
class Frame:
    ...
    def first(self):
        ...
        self.phi = self.x
        self.phi = self.phi.map_coefficients(lambda c:c if c.valuation() is infinity else c.lift_to_precision(c.parent().precision_cap()+c.valuation()))
        self._expansion_of_Phi()
        if not self.phi_divides_Phi():
    ...
    def _expansion_of_Phi(self,length=infinity):
        ...
        q, r = self.Phi._quo_rem_hensel(self.phi)
        self._phiexppoly.append(r)
        while q != 0 and length > len(self._phiexppoly):
            q, r = q._quo_rem_hensel(self.phi)
            self._phiexppoly.append(r)
