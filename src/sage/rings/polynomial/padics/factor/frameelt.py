...
class FrameElt:
    ...
    def __init__(self,frame,a=None,this_exp=None):
        ...
        if this_exp != None:
            ...
            assert denominator(this_exp) == 1
            this_exp = numerator(this_exp)
    ...
    def residue(self):
        ...
        if self.frame.is_first():
            if self.rep[0]._exponent == 0:
                ...
                return self.frame.R(self.rep[0]._unit.residue())
            else:
    ...
    def polynomial(self,denominator=False):
        if denominator:
            ...
            if self.frame.is_first():
                return self.frame.O.uniformizer()**int(min(self.rep[0]._exponent,self.frame.O.precision_cap()))*self.frame.O(self.rep[0]._unit).lift_to_precision(self.frame.O.precision_cap()) # fix precision
            else:
...
def _phi_expansion(P,phi):
    ...
    if phi.degree() > P.degree():
        return [P]
    expansion = []
    q, r = P._quo_rem_hensel(phi)
    expansion.append(r)
    while q != 0:
        q, r = q._quo_rem_hensel(phi)
        expansion.append(r)
    return expansion
...
class FrameEltTerm:
    ...
    def __init__(self,frelt,a,e):
        ...
        if is_Element(a) and  (a in self.frameelt.frame.Ox or a in self.frameelt.frame.O):
    ...
    def reduce(self):
        ...
        if self._exponent >= Eplus * Fplus:
            q,r = int(self._exponent)._quo_rem_hensel(int(Eplus))
            self._exponent = r
